# 帧跳转超链接画布元素 / Hyperlink-style frame jump canvas item.
try:
...
class RefFrameJumpItem(QtWidgets.QGraphicsTextItem):
    ...
    def _jump_to_frame(self):
        if nuke is None:
            QtWidgets.QMessageBox.information(None, "Nuke Unavailable", "Frame jump is only available inside Nuke.")
            return
        try:
            nuke.frame(int(self.frame))
        except Exception:
            QtWidgets.QMessageBox.information(
                None,
                "Frame Jump Failed",
                "This frame could not be opened in the current Nuke session.",
            )

    def to_model(self):
        return FrameJumpModel(
            frame=self.frame,
    ...
    def from_model(cls, model):
        item = cls(model.frame, model.display_label)
